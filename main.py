        ...
        with open(self.sLogFileName, "w") as external_file:
            try:
                ...
                for self.command in self.lCommands:
                    self.DateTimeStart = datetime.datetime.now()
                    print(f"\n>---time start: {self.DateTimeStart} -----")
                    print(f"\n>---time start: {self.DateTimeStart} -----", file=external_file)
                    ...
                    print(f"----command time left: {self.TimerStartStop} sec----<\n", file=external_file)
                    #time.sleep(0.1)
                print(f"\n\Log files: {self.sLogFileName}")
                print(f"\n\Log files: {self.sLogFileName}", file=external_file)
                return True
...
class ecrssh:
    ...
    # ------ Определение интерфейсного объекта ------ #
        ...
        self.menu_def:list = [['&File', ['!&Open...', '&Save', '---', 'E&xit']],
                         ['&Devices', ['&Append...          <F5>', 'Append &copy...  <F6>', '---', '&Edit...               <F4>', '---', '&Delete'], ],
                         ['&Run', ['&Mark/Unmark     <Space>', '---', '&Start                  <F2>', 'Start &ONE          <F3>']],]
        # ------ Определение полей ввода/вывода ------ #
        

        #self.col1 = sg.Column([[sg.Frame('Information:', [], size=(235,350), pad=(0,0))]])], ], pad=(0,0))
        
##        self.col3 = sg.Column([[
##          sg.Frame('Actions:',[[sg.Button('Delete')
##                              ]], size=(450,45), pad=(0,0))
##                              ]])
##        ]], pad=(0,0))
        ...
        self.layout = [[self.sg.Menu(self.menu_def)],
                       ...
                       [self.sg.Button('Append...', key='--APPEND--'), self.sg.Button('Append copy...', key='--APPENDCOPY--'),
                        ...
                        self.sg.Text('                             '),
                        self.sg.Button('Start ONE', key='--START1--')],
                       ...
                       [self.sg.Text('', relief=self.sg.RELIEF_SUNKEN, size=(25, 1), pad=(0, 3), key='-records-'),
                        self.sg.Text(self.msg1, relief=self.sg.RELIEF_SUNKEN, size=(112, 1), pad=(0, 3), key='-status-'),
    ...
    ## ------ Обработка поступивших событий от элементов управления------ #
        ...
        # execute только при запуске для получения фокуса таблицей
           ...
           self.fStart = False
            ...
            #print(self.cValue['-DEVICE-'], self.cValue['-DEVICE-'][0])
            #self.refresh()
        ...
        #переход в начало таблицы
            self.recno = 0
            self.refresh()
        elif self.cEvent in ['End:115', 'End:36']:
